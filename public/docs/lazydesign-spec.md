...
- `v0.1` established the design language specification.
- `v0.2` introduced the core token and theme runtime.
- `v0.2.1` stabilizes the public token contract before component runtime begins.
...
Short variables such as `--ld-primary` and `--ld-radius-md` are compatibility aliases only. New components should consume the namespaced contract.
